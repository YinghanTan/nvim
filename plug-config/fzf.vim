...
let g:fzf_commits_log_options = '--graph --color=always --format="%C(auto)%h%d %s %C(white)%C(bold)%cr %C(auto)%C(blue)%an"'
...
let $FZF_DEFAULT_OPTS = '--layout=reverse --info=inline --border --bind="ctrl-a:select-all,ctrl-A:deselect-all,ctrl-u:preview-half-page-up,ctrl-d:preview-half-page-down" --preview-window="right:50%" --preview="bat --style=numbers --color=always --line-range :500 {}"'
